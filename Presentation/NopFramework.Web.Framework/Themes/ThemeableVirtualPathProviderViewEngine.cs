...
namespace NopFramework.Web.Framework.Themes
{
    ...
    public abstract  class ThemeableVirtualPathProviderViewEngine: VirtualPathProviderViewEngine
    {
        ...
        protected virtual string CreateCacheKey(string prefix, string name, string controllerName,
            string areaName, string theme)
        {
            ...
            string theme, string cacheKeyPrefix, bool useCache, out string[] searchedLocations)
        {
            ...
            if (!viewLocations.Any())
            {
                throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture,
                    "Properties cannot be null or empty - {0}", locationsPropertyName));
            }
            bool nameRepresentsPath= IsSpecificPath(name);
            string cacheKey= CreateCacheKey(cacheKeyPrefix, name, (nameRepresentsPath) 
                ? String.Empty : controllerName, areaName, theme);
            if (useCache)
            {
        ...
        //protected virtual string GetPath(ControllerContext controllerContext, string[] location,
        //    string[] areaLocations, string locationsPropertyName, string name, string controllerName,
        //    string theme, string cacheKeyPrefix, bool useCache, out string[] searchedLocations)
        //{
